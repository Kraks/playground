...
object Constraint {
  ...
  def minWithIndex[T](lst: List[T])(using ordering: Ordering[T]): (T,Int) = {
    assert(lst.nonEmpty)
...
trait Constraint[C <: Constraint[C]] extends Term {
  ...
  def minCoef: ((Int, String), Int) = {
    val (v, idx) = minWithIndex(coefficients.tail)(using Ordering.by((x:Int) => abs(x)))
    ((v, getVarByIdx(idx+1)), idx+1)
  }

  def minCoefUnprotected(pvars: List[String]): ((Int, String), Int) = {
    val (v, idx) = minWithIndex(coefficients.tail.filter(!pvars.contains(_)))(using Ordering.by((x:Int) => abs(x)))
    ((v, getVarByIdx(idx+1)), idx+1)
  }
...
case class Problem(cs: List[Constraint[_]], pvars: List[String] = List(), substs: List[Subst] = List()) {
  ...
  def elimEq: Option[Problem] = {
    ...
    def eliminate(eqs: List[EQ], geqs: List[GEQ], substs: List[Subst]): Option[Problem] = {
      if (eqs.nonEmpty) {
        ...
        if (debug) {
          for (eq <- (eqs/*++geqs*/)) do println(s"  $eq")
          for (eq <- (geqs)) do println(s"  $eq")
        }
  ...
  def chooseVarMinCoef: String = {
    val ((c, x), _) = minWithIndex(cs.map(_.minCoef._1))(using Ordering.by({
  ...
  def chooseUnprotectedVarMinCoef: Option[String] = {
    ...
    if (coefVars.nonEmpty) {
      val ((c, x), _) = minWithIndex(coefVars)(using Ordering.by({
        case x: (Int,String) => abs(x._1)
      }))
      Some(x)
    }

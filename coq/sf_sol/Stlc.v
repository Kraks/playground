...
Notation idBBBB :=
  (tabs x (TArrow (TArrow TBool TBool) (TArrow TBool TBool))
    (tvar x)).
...
Inductive substi (s:tm) (x:id) : tm -> tm -> Prop :=
  ...
  | s_abs1 : forall T t, substi s x (tabs x T t) (tabs x T t)
...
Proof.
  ...
  - inversion H as [S' H1]. inversion H1. apply IHT1. exists T2. assumption.
Qed.

Example typing_nonexample_3 :
  ~ (exists S, exists T,
      empty |-
        (tabs x S (tapp (tvar x) (tvar x))) \in
        T).
Proof.
  ...
  apply no_infinite_nested_type. exists T11. exists T12.
  subst. apply H4.
Qed.
(** [] *)
